...
public class ReturnPercent {
    ...
    public static<T> double partOf(List<T> nameOfList, Predicate<T> predicate) {
        return (double) nameOfList.stream().filter(predicate).count() / nameOfList.size();
